...
from .trans_decoder import TransBasicLayer, PyramidPoolAgg, InjectionMultiSumCBR
from .Slinding_MLP import OverlapPatchEmbed, SlindingBlock
...
class encoder(nn.Module):
    def __init__(self,
        ...
        self.patch_embed4 = OverlapPatchEmbed(img_size=pretrain_img_size[0] // 8, patch_size=3, stride=2, in_chans=int(embed_dim * 2 ** 2),
                                              embed_dim=int(embed_dim * 2 ** 3))
        self.norm4 = nn.LayerNorm(int(embed_dim * 2 ** 3))
        self.block2 = nn.ModuleList([SlindingBlock(
            dim=int(embed_dim * 2 ** 3), num_heads=1, mlp_ratio=1, qkv_bias=False, qk_scale=None,
...
class decoder(nn.Module):
    def __init__(self,
        ...
        self.channels = [embed_dim*2**0, embed_dim*2**1, embed_dim*2**2]
    ...
    def forward(self, x, skips):
        ...
        x = self.pos_drop(x)
        dec3, H, W = self.decoder3(x, skips[2], H, W)
        dec2, H, W = self.decoder2(dec3, skips[1], H, W)
        dec1, H, W = self.decoder1(dec2, skips[0], H, W)
...
class Block(nn.Module):
    def __init__(self, k, dim, drop_path=0., layer_scale_init_value=1e-6, input_resolution=None, num_heads=None,
                 window_size=None, i_block=None, qkv_bias=None, qk_scale=None):
        super().__init__()
        self.channels = dim
        pad = k // 2
        in_dim = dim//2
        self.dwconv = nn.Conv2d(in_dim, in_dim, kernel_size=k, padding=pad, groups=in_dim)  # depthwise conv
        self.norm = LayerNorm(in_dim, eps=1e-6)
        self.pwconv1 = nn.Linear(in_dim, 4 * in_dim)  # pointwise/1x1 convs, implemented with linear layers
        self.act = nn.GELU()
        self.pwconv2 = nn.Linear(4 * in_dim, in_dim)
    ...
    def forward(self, x, mask):
        input = x
        a, b = x.split(self.channels // 2, dim=1)
        a = self.dwconv(a)
        a = a.permute(0, 2, 3, 1)  # (N, C, H, W) -> (N, H, W, C)
        a = self.norm(a)
        ...
        # x = input + torch.cat([self.drop_path(a), out], 1)
        x = input + torch.cat([a, out], 1)
...
class AERFNet(SegmentationNetwork):
    def __init__(self,
                 config,
                 num_input_channels,
                 embedding_dim,
