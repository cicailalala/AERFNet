...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.patience = 50
        self.val_eval_criterion_alpha = 0.9#0.9  # alpha * old + (1-alpha) * new
        ...
        self.best_val_eval_criterion_MA = 0.0001
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                #print('batch1')
                #print(self.tr_gen)
                with tqdm(self.tr_gen) as tbar:
                    for batch in tbar:
                        print('batch')
                        tbar.set_description("Epoch {}/{}".format(self.epoch+1, self.max_num_epochs))
                        if len(self.val_list)!=0:
                            l = self.run_iteration(batch, True)
                        else:
                            l = self.run_iteration(batch, True,True)
                        
                        #self.lr_scheduler.step_update(self.epoch * self.num_batches_per_epoch + idx)
                        idx+=1
            ...
            if len(self.val_list)!=0:
                ...
                val_losses = []
                with tqdm(self.val_gen) as tbar:
                    for batch in tbar:          
                        l = self.run_iteration(batch, False, True)
                        val_losses.append(l)
            if len(self.val_list)!=0:
