...
from nnunet.network_architecture.AERFNet import AERFNet
...
class nnUNetTrainerV2_aerfnet_acdc(nnUNetTrainer):
    """
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        if self.model_size == 'Tiny':
            ...
            if self.pretrain:
                self.pre_trained_weight = torch.load("/group/program/aerfnet/our_weight/imagenet_tiny.model",map_location='cpu')
            
        if self.model_size=='Base':
            self.embedding_dim = 128
            self.num_heads = [4,8,16,32]
            if self.pretrain:
                self.pre_trained_weight = torch.load("/group/program/aerfnet/our_weight/imagenet_base.model",map_location='cpu')
