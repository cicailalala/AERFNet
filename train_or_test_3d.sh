...
then
	...
	ulimit -n 4000
	CUDA_VISIBLE_DEVICES=${cuda} nnUNet_train 3d_fullres nnUNetTrainerV2_${name} ${task} 0
fi

if ${predict}
then
	cd $nnUNet_raw_data_base/nnUNet_raw_data/$target_folder/
	CUDA_VISIBLE_DEVICES=${cuda} nnUNet_predict -i imagesTs -o inferTs/${name}_${step}step -m 3d_fullres -t ${task} -chk model_best -tr nnUNetTrainerV2_${name}
	...
	python evaluate.py ${name}_${step}step
fi
